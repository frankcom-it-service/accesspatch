...
## 2026-07-16 — Phase 3A Deterministic Judge Workflow

- Goal: provide `pnpm judge:verify` after the separate `pnpm install --frozen-lockfile` prerequisite, without API access, evidence regeneration, ignored-run dependency, or tracked evidence modification.
- Architecture: `scripts/judge-workflow.ts` defines eight ordered stages, repository-contract validation, minimal child environment, external-network guards, repository/sample snapshots, and cleanup; `scripts/judge-verify.ts` is the CLI entry point.
- Baseline distinction: `pnpm test:baseline` remains intentionally non-zero; `pnpm test:judge-baseline` exits `0` only for the exact two controlled defects, one `label` violation on `#email`, focus values `none` / `3px` / `none` / `false`, and confirmation reached.
- Tests: 25 new API-free tests; 139 total. They cover plan order, failure propagation, root and contract rejection, sample modification, ignored-run independence, isolated runtime configuration, fake-home Git and pnpm checks, cleanup, stable summary fields, and baseline mutations.
- Offline validation: frozen install completed in 452 ms; no floating versions; high-severity audit found no known vulnerabilities; license groups remained Apache-2.0, BSD-3-Clause, ISC, MIT, and MPL-2.0. Type-check, build, smoke, both baseline modes, sample validation, tracked-report smoke, diff, whitespace, credential, identifier, absolute-path, and claim scans passed.
- Authorized full run: exactly one `pnpm judge:verify` execution ran with `.accesspatch/runs/` absent. Stage 1 passed; stage 2 failed because 31 pre-existing proof-bundle tests still copied repository ignored artifacts. The workflow exited `1`; no stable success summary was emitted; no retry occurred.
- Corrections after failure: proof-bundle test fixtures now use the tracked Judge Sample and reconstruct the reviewed evidence byte-for-byte with the approved hash. Independent corrected-source review passed that change in archive SHA-256 `2f328fbe97d1088945ba9291b8669f5bdd768137196b800356a779ba535df7a5`, then identified incomplete Git/npm home-configuration isolation. The workflow now removes stale reserved output, creates empty restricted `gitconfig` and `npmrc` files below `.accesspatch/work/judge-verify/runtime-config/`, redirects Git global/system configuration and npm/pnpm user/global configuration, disables prompts and interactive credential handling, and guarantees cleanup after preflight or stage failure. The corrected stage passed 139 tests with ignored runs absent.
- Corrected proof run: the final independent pre-run review archive SHA-256 was `1e458514f2737234765705a7063afa8658c8e887233a727d91e45eb05c2ec9b9`. The second lifetime `pnpm judge:verify` execution exited `0`; no retry occurred. All eight stages started and passed exactly once, 139 tests passed, the controlled baseline reproduced exactly both findings and one `label` target `#email`, confirmation was reached, and the tracked report returned zero axe violations.
...
## 2026-07-16 — Phase 5A Accessibility Mutation Guard

- Phase goal: demonstrate one deterministic accessibility regression detector without changing the retained fixture or broadening repository support.
- Target: the unique native checkout submit button selected by `.controlled-focus-defect`; it is stable, journey-critical, enabled, visible, and keyboard-focusable.
- Mutation: inject `aria-hidden="true"` exactly once in a disposable four-file checkout copy; reject zero, duplicate, or already-mutated targets.
- Detection: browser-observed active-element focus, native button state, tab index, visibility, and `aria-hidden` state drive rule `FOCUSABLE_ELEMENT_ARIA_HIDDEN`; axe is not the authoritative detector.
- Isolation: allowlisted regular files only; path traversal, symlinks, credentials, environment files, dependencies, Git metadata, ignored runs, and unrelated repository content are excluded.
- Evidence: ignored versioned `result.json` plus bounded `result.md`; no timestamp is included, enabling stable repeatability comparison.
- Tests: 39 Phase 5A API-free additions, including 20 focused security-hardening and lifecycle tests; 178 total in the full suite.
- Runtime: one initial sandbox-blocked localhost attempt cleaned up safely. During hardening, one initial execution and two diagnostic executions failed closed on Chromium worker termination when the browser profile used repository work storage; no success artifact or retained mutation resulted. After the final lifecycle correction, two passing executions produced identical stable output, byte-identical evidence, one injection, one detection, unchanged original hash, and removed runtime state.
- Security hardening: isolated child HOME/config/cache/temp/application-data directories beneath a unique disposable browser runtime; full `.accesspatch` runtime-chain symlink and containment checks; approved fixture-hash binding; and direct abort-cleanup verification. A final independent review found that Vite was started before fallible detector preparation entered cleanup protection. The lifecycle now completes all preparation before spawn, immediately protects every post-spawn path, sanitizes child-process errors, promptly aborts server waiting, and independently attempts and verifies both cleanup roots before success evidence.
- Completion: **CLEAN-CLONE VERIFIED / COMMITTED ON FEATURE BRANCH** in `b05541f147a08cb0b38ecbda5ae5802db065a3d2`; final independent implementation review passed with archive SHA-256 `d2d63d1c708b57e000fe915092ac70c4f457608eededf0e96757b7a0c56a3c91`.
- Final evidence: 178 API-free tests passed; the full eight-stage Judge Workflow passed after the lifecycle correction; fixture SHA-256 before and after was `50a88ce11e4afc62f66c595b313156f2f1a7f1fc98eb2f6ad1206b8260fbbfd4`; ignored result hashes were `0012aea061beb4d2d5dd7b72728043fa7c5882cf47b625be921186735865b3d0` and `5f382756c9908b45c699556bf787074573f03b24354094567fdc315d0159f2ac`.
- Preservation: original fixture, Proof Bundle contract, Judge Sample, Phase 3B evidence, immutable refs, and external backups remained unchanged. The branch is local-only, `main` remains the independently usable submission version, and merge remains pending separate approval. No GPT call, credential inspection, retained source mutation, push, or remote change occurred.
- Clean-clone acceptance: feature HEAD `d75954b672cbc88569d5502198effc57c4390c63` passed all 17 stages exactly once on Debian GNU/Linux 13 x86_64 with Node `v24.18.0`, pnpm `11.13.0`, Git `2.47.3`, and Chromium `148.0.7778.178`. The one offline frozen installation reused 33 packages and downloaded zero.
- Temporary install control: `trustLockfile: true` existed only in the disposable clone, pnpm recognized it, and original `pnpm-workspace.yaml` SHA-256 `0e04d3437163206aeae97fa421e4fa57546ba9afbb3178a1ba568efa827af35d` was restored byte-for-byte before runtime execution.
...
- Status: **CLEAN-CLONE VERIFIED / COMMITTED ON FEATURE BRANCH**; merge remains separately gated.
